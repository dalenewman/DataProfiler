...
namespace DataProfiles.Autofac {
    public class PipelineRunner : IRun {
        ...
        public IEnumerable<Row> Run(Root root) {
            ...
            nested.RegisterModule(new ContextModule(root));
            nested.RegisterModule(new AdoModule(root));
            ...
            using (var scope = nested.Build().BeginLifetimeScope()) {
                ...
                var controller = scope.ResolveNamed<IPipeline>(root.Processes[0].Entities[0].Key); ;
